class FileType {
  ...
  tryReload = () => {
    if(this.path === 'css') return true;//patch for inteliji idea when you changing some css it changes css folder to
...
if (typeof process === 'undefined') {
  ...
  const watchFunc = (watch, exceptions, io, pathToWatch) => watch(pathToWatch, { recursive: true }, (evt, path) => {
    let include = false;
    ...
    if (!include) {
      ...
      io.emit('change', { evt, path, exceptions });
    }
  });
